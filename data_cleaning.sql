...
-- station table
ALTER TABLE station 
    ADD COLUMN installation_date_temp DATE; -- temporary column
UPDATE station 
    SET installation_date_temp = STR_TO_DATE(installation_date, '%m/%d/%Y') WHERE installation_date IS NOT NULL; -- format conversion
ALTER TABLE station 
    DROP COLUMN installation_date; -- drop old column
ALTER TABLE station 
    CHANGE installation_date_temp installation_date DATE; -- change column name

-- weather table
ALTER TABLE weather 
    ADD COLUMN date_temp DATE;
UPDATE weather 
    SET date_temp = STR_TO_DATE(date, '%m/%d/%Y') WHERE date IS NOT NULL;
ALTER TABLE weather 
    DROP COLUMN date;
ALTER TABLE weather 
...
-- trip table
ALTER TABLE trip 
    ADD COLUMN (start_date_temp TIMESTAMP, end_date_temp TIMESTAMP);
UPDATE trip
